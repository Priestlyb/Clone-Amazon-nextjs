// components/CheckoutProducts.js
...
function CheckoutProducts({
  ...
  const removeItemFromBasket = () => {
    dispatch(removeFromBasket({ index })); // <- Use index to remove
  };
  ...
  return (
    <div className="grid grid-cols-5">
      ...
      <div className="col-span-1 space-y-2 my-2 justify-self-end">
        <button onClick={addItemToBasket} className="w-full button">
          Add to Basket
        </button>
        <button onClick={removeItemFromBasket} className="w-full button">
          Remove from Basket
        </button>
      </div>
    </div>
